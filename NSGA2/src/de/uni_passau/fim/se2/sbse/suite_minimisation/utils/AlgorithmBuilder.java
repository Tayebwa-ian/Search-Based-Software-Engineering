...
import de.uni_passau.fim.se2.sbse.suite_minimisation.algorithms.Nsga2Algorithm;
...
public class AlgorithmBuilder {
    ...
    private MinimizingFitnessFunction<? extends Chromosome<?>> makeTestSuiteSizeFitnessFunction() {
        TestCaseMinimizationFitness fitnessFunction = new TestCaseMinimizationFitness();
    ...
    private MaximizingFitnessFunction<? extends Chromosome<?>> makeTestSuiteCoverageFitnessFunction() {
        CoverageMaximizationFitness fitnessFunction =  new CoverageMaximizationFitness(coverageMatrix);
        return fitnessFunction;
    }
    ...
    private GeneticAlgorithm<? extends Chromosome<?>> buildNSGA2() {
        Comparator<Encoding> comparator = new Comparator<Encoding>() {
            @Override
        ...
        GeneCrossover crossover = new GeneCrossover(random);
        BitFlipMutation mutation = new BitFlipMutation(random);
        EncodingGenerator encodingGenerator = new EncodingGenerator(random, mutation, crossover, numberTestCases, coverageMatrix);
        Nsga2Algorithm algorithm = new Nsga2Algorithm(stoppingCondition, encodingGenerator, selection, random);
        return algorithm;
    }
    ...
    private GeneticAlgorithm<? extends Chromosome<?>> buildRandomSearch() {
        ...
        BitFlipMutation mutation = new BitFlipMutation(random);
        EncodingGenerator encodingGenerator = new EncodingGenerator(random, mutation, crossover, numberTestCases, coverageMatrix);
        RandomSearch randomSearch = new RandomSearch(encodingGenerator, stoppingCondition);
        return randomSearch;
    }
}
