...
public class Main implements Callable<Integer> {
    ...
    private static Tasks globalTask;
    ...
    @Override
    public Integer call() {
        globalTask = this.task;  // Save the CLI-provided task in a static field
        System.out.printf(
        ...
        IntStream.range(0, repetitions).forEach(i -> solveTask());
        printResults();
    ...
    private void solveTask() {
        ...
        System.out.println("The solution fitness is: " + solution.getFitness() + " and the result is: " + environment.solved(solution));
    ...
    public static Neuroevolution initialiseNeat(int populationSize, int maxGenerations) {
        ...
        Environment env = new SinglePoleBalancing(10, true, Randomness.random());
        NetworkGenerator generator = new NetworkGenerator(innovations, env.stateSize(), env.actionInputSize(), random);
        NEAT alogrithm = new NEAT(
            random,
            crossover,
            mutation,
            generator,
    ...
    private Environment initialiseTask() {
        // Use the globalTask if this.task is null.
        Tasks taskToUse = (this.task != null) ? this.task : globalTask;
        return switch (taskToUse) {
            case XOR -> new XOR();
            case CARTPOLE -> new SinglePoleBalancing(10, false, Randomness.random());
            case CARTPOLE_RANDOM -> new SinglePoleBalancing(10, true, Randomness.random());
        };
    }
