...
public final class BranchTracer implements IBranchTracer {
    ...
    public void passedBranch(final int i, final int opcode, final int trueBranch, final int falseBranch) {
        ...
        switch (opcode) {
            case IFEQ:
                ...
                distanceFalse = (i != 0) ? 0 : 1;
                break;
            case IFNE:
                // i != 0 (true), i == 0 (false)
                distanceTrue = (i != 0) ? 0 : 1;
                distanceFalse = Math.abs(i);
                break;
            case IFLT:
                // i < 0 (true), i >= 0 (false)
                distanceTrue = (i < 0) ? 0 : i + 1;
                distanceFalse = (i >= 0) ? 0 : Math.abs(i);
                break;
            case IFLE:
                distanceTrue = (i <= 0) ? 0 : i;
                distanceFalse = (i > 0) ? 0 : Math.abs(i) + 1;
                break;
            case IFGT:
                distanceTrue = (i > 0) ? 0 : Math.abs(i) + 1;
                distanceFalse = (i <= 0) ? 0 : Math.abs(i);
                break;
            case IFGE:
                distanceTrue = (i >= 0) ? 0 : Math.abs(i);
                distanceFalse = (i < 0) ? 0 : i + 1;
                break;
            default:
    ...
    public void passedBranch(final int i, final int j, final int opcode, final int trueBranch,
        ...
        switch (opcode) {
            case IF_ICMPEQ:
                distanceTrue = (i == j) ? 0 : Math.abs(i - j);
                distanceFalse = (i != j) ? 0 : 1;
                break;
            case IF_ICMPNE:
                distanceTrue = (i != j) ? 0 : 1;
                distanceFalse = (i == j) ? 0 : Math.abs(i - j);
                break;
            case IF_ICMPLT:
                distanceTrue = (i < j) ? 0 : i - j + 1;
                distanceFalse = (i >= j) ? 0 : j - i;
                break;
            case IF_ICMPLE:
                distanceTrue = (i <= j) ? 0 : i - j;
                distanceFalse = (i > j) ? 0 : j - i + 1;
                break;
            case IF_ICMPGT:
                distanceTrue = (i > j) ? 0 : j - i + 1;
                distanceFalse = (i <= j) ? 0 : i - j;
                break;
            case IF_ICMPGE:
                distanceTrue = (i >= j) ? 0 : j - i;
                distanceFalse = (i < j) ? 0 : i - j + 1;
                break;
            default:
                throw new IllegalArgumentException("Unsupported opcode: " + opcode);
        }
    ...
    public void passedBranch(final Object o, final int opcode, final int trueBranch,
        ...
        switch (opcode) {
            case IFNULL:
                distanceTrue = (o == null) ? 0 : 1;
                distanceFalse = (o != null) ? 0 : 1;
                break;
            case IFNONNULL:
                distanceTrue = (o != null) ? 0 : 1;
                distanceFalse = (o == null) ? 0 : 1;
    ...
    public void passedBranch(final Object o, final Object p, final int opcode,
        ...
        switch (opcode) {
            case IF_ACMPEQ:
                distanceTrue = (o == p) ? 0 : 1;
                distanceFalse = (o != p) ? 0 : 1;
                break;
            case IF_ACMPNE:
                distanceTrue = (o != p) ? 0 : 1;
                distanceFalse = (o == p) ? 0 : 1;
    ...
    private void traceBranchDistance(final int trueBranch, final double distanceTrue, final int falseBranch, final double distanceFalse) {
        double currentTrue = distances.getOrDefault(trueBranch, Double.POSITIVE_INFINITY);
        double currentFalse = distances.getOrDefault(falseBranch, Double.POSITIVE_INFINITY);

        if (distanceTrue < currentTrue) {
            distances.put(trueBranch, distanceTrue);
        }
        if (distanceFalse < currentFalse) {
    ...
    /**
     * Returns the distance of a branch in a given testCase
     * @param branchId the ID of the branch
     * @return the distance of a branch in a given testCase
     */
    public double getBranchDistance(int branchId) {
        return distances.getOrDefault(branchId, Double.POSITIVE_INFINITY);
    }
}
